<?php
    $app->delete('/v1/establecimiento/500/establecimiento/{codigo}', function($request) {
        ...
        if (isset($aud01) && isset($aud02) && isset($aud03) && isset($aud04) && isset($val01)) {
            $sql00  = "UPDATE ESTFIC SET ESTFICAEM = ?, ESTFICAUS = ?, ESTFICAFH = ?, ESTFICAIP = ? WHERE ESTFICCOD = ?";
            $sql01  = "DELETE FROM ESTFIC WHERE ESTFICCOD = ?";
            ...
            try {
                ...
                header("Content-Type: application/json; charset=utf-8");
                $json                   = json_encode(array('code' => 204, 'status' => 'failure', 'message' => 'Error DELETE: No se puede eliminar, existe informaciones relacionadas. Verifique!'), JSON_UNESCAPED_UNICODE | JSON_NUMERIC_CHECK | JSON_PRESERVE_ZERO_FRACTION);
            }
        } else {
            header("Content-Type: application/json; charset=utf-8");
